 ...
 */
function sanitizeForSms(v: unknown, maxLen = 40): string {
  if (v == null) return '';
  let s = String(v)
 ...
 */
async function loadRecipientPrefs(userId: string | null | undefined): Promise<RecipientPrefs | null> {
  ...
  if (!data) return null;
  if (data.smsNotificationsEnabled !== true) return null;
...
async function send({ toE164, body }: SendArgs): Promise<void> {
...
export async function sendMatchSms(
  ...
  const body =
    `Campus Connections: You've been matched with ${partnerName} for your ${airport} trip. ` +
...
export async function sendNoMatchSms(recipient: TripRequest): Promise<void> {
  const prefs = await loadRecipientPrefs(recipient.userId);
  if (!prefs) return;

  const airport = sanitizeForSms(recipient.departingAirport, 10);
  const body =
    `Campus Connections: No match yet for your ${airport} trip. ` +
...
export async function sendXlRideSuggestionSms(recipient: TripRequest): Promise<void> {
  const prefs = await loadRecipientPrefs(recipient.userId);
  if (!prefs) return;

  const airport = sanitizeForSms(recipient.departingAirport, 10);
  const body =
    `Campus Connections: Your combined luggage for your ${airport} trip may not fit a standard rideshare. ` +
